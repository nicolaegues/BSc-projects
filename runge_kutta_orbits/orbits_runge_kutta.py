...
epsilon = 1e-10 #add to zeros to avoid DivisionError
...
def f_2_with_moon(r): 
    ...
    dv_dt = -(G*Me*re)/((re[0]**(2)+ re[1]**(2) + re[2]**2)**(3/2)) - (G*Mm*rm)/((rm[0]**(2)+ rm[1]**(2) + rm[2]**2)**(3/2))
    return dv_dt
...
def plot_2Dorbit(x_vals, y_vals, x2_vals = [], y2_vals = [], moon = False, Sun = False, zoom = False): 
    ...
    if moon == False: 
        ...
        if len(x2_vals) == 0:
            ...
            plt.plot(Re_0[0], Re_0[0], "o",c= "blue", label = "Earth" )
            plt.plot(Rm_0[0], Rm_0[1], "o", c = "r", label = "Moon")
            plt.plot([], [], c = "black", label = "Rocket")
            plt.title("2D Simulation of Rocket's orbit around Earth and Moon")

        else: 

            plt.plot(Re_0[0], Re_0[0], "o",c= "b", label = "Earth" )
            plt.plot(Rm_0[0], Rm_0[1], "o", c = "r", label = "Moon")
        ...
        if Sun == True: 
            ...
            plt.plot(Rs_0[0], Rs_0[1], "o",  c = "y", label = "Sun")
...
def plot_3Dorbit(x_vals, y_vals, z_vals, x2_vals = [], y2_vals = [], z2_vals = [],moon = False, Sun = False, zoom = False): 
    ...
    if moon == False: 
        ...
        #i.e part b
            ...
            plt.plot(Re_0[0], Re_0[0], Re_0[2], "o",c= "b", label = "Earth" )
            plt.plot(Rm_0[0], Rm_0[1], Rm_0[2], "o", c = "r", label = "Moon")
            plt.plot([], [], [], c = "black", label = "Rocket")
        ...
        #i.e part c and d
            ...
            if Sun == True: 
                ...
                plt.plot(Rs_0[0], Rs_0[1], Rs_0[2], "o",  c = "y", label = "Sun")
...
def plot_2Danimated_orbit(x_vals, y_vals, dt, x2_vals = [], y2_vals = [], moon = False, Sun = False, zoom = False, part = ""): 
    """"
    ...
    frame_factor = int(len(x_vals)/nr_frames)
    ...
    def animate(frame):
        ...
        line.set_data(x_vals[:step], y_vals[:step])
        line2.set_data(x2_vals[:step], y2_vals[:step])
        circle.set_data([x_vals[step-1]], [y_vals[step-1]])
        #time_text.set_text(f'Time: {(seconds):.2f} Seconds, {(hours):.2f} Hours, {(days):.2f} Days')
        if len(x2_vals) != 0:
            circle2.set_data([x2_vals[step-1]], [y2_vals[step-1]])
            return line, line2, circle, circle2, 
        # time_text,
        else: 
            return line, circle, 
        #time_text,
        
    frames = len(x_vals)//frame_factor
    anim = animation.FuncAnimation(fig, animate, frames = nr_frames, interval = 1, blit = True )
    anim.save(f"{current_dir}/2D_orbit_part_{part}.gif")
    plt.show()
    ...
    if MyInput == 'a':
        ...
        while True: 
            ...
            if p_input == "plot2":
                ...
                plot_2Danimated_orbit(x[:int(len(x)*0.2)], y[:int(len(x)*0.2)], dt, part=MyInput)
        ...
        while True: 
            ...
            if p_input == "plot2":
                plot_2Danimated_orbit(x, y, dt, moon = True, part = MyInput)
                print("\nIf a blank plot appears, please rerun - matplotlib does that sometimes with the animation. Or, if you have VSCode, open it in VSCode\n")
        ...
        earth_r_start_vals = np.array([0, 0, 0]) + epsilon
        ...
        while True: 
            ...
            if p_input == "plot2":
                plot_2Danimated_orbit(xe, ye, dt, xm, ym, moon = True, part = MyInput)
                print("\nIf a blank plot appears, please rerun - matplotlib does that sometimes with the animation. Or, if you have VSCode, open it in VSCode\n")
        ...
        #plotting options
        while True: 
            p_input = input('Type "plot1" if you want to plot the 2D orbit, "plot2" if you want to zoom in, "plot3" if you want the animated zoom-in, "plot4" if you want to plot the orbit in 3D or press Enter to skip: ')
            ...
            if p_input == "plot3":
                lenx = len(xe)
                plot_2Danimated_orbit(xe[:int(len(xe)*0.1)], ye[:int(len(xe)*0.1)], dt, xm[:int(len(xe)*0.1)], ym[:int(len(xe)*0.1)], moon = True, Sun = True, zoom = True, part=MyInput)
